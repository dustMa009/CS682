...
// to run this file, copy PositionChanger.java to the same directory.
public class MolComSim {
	...
	// divide the medium into left and right part
	private void run(String[] args)  throws IOException {
		startSim(args);
		int mediumLength = medium.getLength();
		PositionChanger left = new PositionChanger((int)- mediumLength / 2, 0);
		PositionChanger right = new PositionChanger(0, (int) mediumLength / 2 + 1);
		ExecutorService executor = Executors.newFixedThreadPool(2);
		Future<PositionChanger> future1, future2;
		...
		for(; (simStep.get() < simParams.getMaxNumSteps()) && (!lastMsgCompleted.get()); simStep.incrementAndGet())
		{
			...
			future1 = executor.submit(left);
			future2 = executor.submit(right);
			try {
				future1.get();
				future2.get();
			} catch (Exception e) {
